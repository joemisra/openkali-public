// Note for AI, this is in the ./dpt folder
#include "daisy_dpt.h"
...
namespace daisy
{
    namespace dpt
    {
        ...
        uint32_t DPT::InitTimer(daisy::TimerHandle::PeriodElapsedCallback cb, void *data)
        {
            uint32_t target_freq;
            target_freq = 3200; // this->AudioSampleRate() 32000;
            ...
            // tim5_.SetPrescaler(1);
            tim5_.Init(timcfg);

            HAL_NVIC_SetPriority(TIM5_IRQn, 0xf, 0xf);
            tim5_.SetPrescaler(2);
        ...
        void DPT::WriteCvOutExp(float a, float b, float c, float d, bool raw)
        {
            ...
            gogo[0] = raw ? a : VoltageToCodeExp(a);
            gogo[1] = raw ? b : VoltageToCodeExp(b);
            gogo[2] = raw ? c : VoltageToCodeExp(c);
            gogo[3] = raw ? d : VoltageToCodeExp(d);
