...
float Oscillator::Process()
{
    ...
    float workingphase_ = phase_ + (Offset * TWOPI_F);

    if(lock_) {
        workingphase_ = 0.0f;
    } 
    
    if(workingphase_ > TWOPI_F)
    {
        workingphase_ -= TWOPI_F;
    }
    
    switch(waveform_)
    {
        case WAVE_SIN: out = sinf(workingphase_); break;
        case WAVE_TRI:
            t   = -1.0f + (2.0f * workingphase_ * TWO_PI_RECIP);
            out = 2.0f * (fabsf(t) - 0.5f);
            break;
        case WAVE_SAW:
            out = -1.0f * (((workingphase_ * TWO_PI_RECIP * 2.0f)) - 1.0f);
            break;
        case WAVE_RAMP: out = ((workingphase_ * TWO_PI_RECIP * 2.0f)) - 1.0f; break;
        case WAVE_SQUARE: out = workingphase_ < pw_rad_ ? (1.0f) : -1.0f; break;
        case WAVE_POLYBLEP_TRI:
            t   = workingphase_ * TWO_PI_RECIP;
            out = workingphase_ < PI_F ? 1.0f : -1.0f;
        ...
        case WAVE_POLYBLEP_SQUARE:
            t   = workingphase_ * TWO_PI_RECIP;
            out = workingphase_ < pw_rad_ ? 1.0f : -1.0f;
            out += Polyblep(phase_inc_, t);
